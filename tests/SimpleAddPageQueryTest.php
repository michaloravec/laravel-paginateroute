...
class SimpleAddPageQueryTest extends TestCase
{
    /**
     * @var string
     */
    ...
    public function it_can_append_regular_page_queries_to_the_url()
    {
        ...
        for ($page = 2; $page <= 10; $page++) {
            $this->assertEquals("$url/{$page}", $this->app['paginateroute']->addPageQuery($url, $page));
        }
    }
    ...
    public function it_appends_a_query_for_the_first_page_if_a_full_url_is_requested()
    {
        ...
        $this->assertEquals("$url/{$page}", $this->app['paginateroute']->addPageQuery($url, $page, true));
    }
}
